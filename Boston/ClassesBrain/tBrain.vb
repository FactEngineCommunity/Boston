    ...
    Private Function CheckAskingForHelp(ByVal asInputString As String) As Boolean
        ...
        Select Case LCase(Trim(asInputString))
            Case Is = "help"
                ...
                Dim lrQuestion As New tQuestion("Would you like me to open Help at the respective topic?", _
                                                 pcenumQuestionType.OpenHelpFile, _
                                                 True, _
                                                 Nothing, _
                                                 Nothing, _
                                                 Nothing, _
...
#Region "QUESTIONING"
    ...
    Private Function QuestionHasBeenRaised(ByVal arQuestion As tQuestion) As Boolean
        ...
        If Me.Question.Exists(AddressOf arQuestion.Equals) Then
            Return True
        Else
            Return False
        End If
...
#Region "VAQLFUNCTIONS"
    ...
    Private Sub ProcessONEStatement(ByVal asOriginalSentence As String)
        ...
        For liInd = 1 To Me.VAQL.ATMOSTONEStatement.MODELELEMENT.Count

            lrPredicatePart = New Language.PredicatePart()
            lrPredicatePart.SequenceNr = liInd

            lrModelElementNode = Me.VAQL.ATMOSTONEStatement.MODELELEMENT(liInd - 1)
            Me.VAQL.MODELELEMENTClause.PREBOUNDREADINGTEXT = ""
            Me.VAQL.MODELELEMENTClause.POSTBOUNDREADINGTEXT = ""
            Me.VAQL.MODELELEMENTClause.MODELELEMENTNAME = ""
            ...
            If Me.Model.ExistsModelElement(lsModelObjectName) Then
                If Array.IndexOf({pcenumConceptType.ValueType, _
            ...
            If lbProceedToCreateObjectTypeQuestion Then
                ...
                If lbIsLikelyValueType Then
                    ...
                    lrQuestion = New tQuestion("Would you like me to create an Value Type for '" & lsModelObjectName & "'?", _
                                                     pcenumQuestionType.CreateValueType, _
                                                     True, _
                                                     lasSymbol, _
                                                     Nothing, _
            ...
            lrPredicatePart.ObjectName = Me.VAQL.MODELELEMENTClause.MODELELEMENTNAME

            Dim lsPredicatePartText As String = ""

            If Me.VAQL.ATMOSTONEStatement.UNARYPREDICATEPART = "" Then
                '----------------------------------------
                'FactType is binary or greater in arity
                '----------------------------------------
            ...
            Else
                ...
                'FactType is a unary FactType
                '------------------------------
                lrPredicatePart.PredicatePartText = Trim(Me.VAQL.ATMOSTONEStatement.UNARYPREDICATEPART)
            End If

            lrSentence.PredicatePart.Add(lrPredicatePart)
        Next 'ModelElement

        lrStep = New Brain.Step(pcenumActionType.CreateFactType, _
                                True, _
                                pcenumActionType.None, _
                                pcenumStepFactTypeAttributes.BinaryFactType, _
        ...
        lsEnumeratedFactTypeReading = Me.CreateEnumeratedFactTypeReadingFromParts(lasModelObjectId, lrSentence) ' Me.VAQL.PREDICATEPARTClause.PREDICATEPART)
        ...
        '=====================================
        'Me.VAQL.ATMOSTONEStatement.MODELELEMENTNAME = New List(Of String)
        'Me.VAQL.ATMOSTONEStatement.PREDICATECLAUSE = New Object
        ...
        'Call Me.VAQL.GetParseTreeTokens(Me.VAQL.PREDICATEPARTClause, Me.VAQL.ATMOSTONEStatement.PREDICATECLAUSE)
        ...
        'lsEnumeratedFactTypeReading = Me.CreateEnumeratedFactTypeReadingFromParts(lasModelElementNames, lrSentence.PredicatePart) ' Me.VAQL.PREDICATEPARTClause.PREDICATEPART)
    ...
    Private Sub ProcessFACTTYPEStatement(ByVal asOriginalSentence As String)
        ...
        Try
            ...
            Me.VAQL.FACTTYPEREADINGStatement.FRONTREADINGTEXT = ""
            Me.VAQL.FACTTYPEREADINGStatement.MODELELEMENT = New List(Of Object)
            Me.VAQL.FACTTYPEREADINGStatement.PREDICATECLAUSE = New List(Of Object)
            ...
            lrSentence.FrontText = Me.VAQL.FACTTYPEREADINGStatement.FRONTREADINGTEXT
            lrSentence.FollowingText = Me.VAQL.FACTTYPEREADINGStatement.FOLLOWINGREADINGTEXT

            For liInd = 1 To Me.VAQL.FACTTYPEREADINGStatement.MODELELEMENT.Count

                lrPredicatePart = New Language.PredicatePart()
                lrPredicatePart.SequenceNr = liInd

                lrModelElementNode = Me.VAQL.FACTTYPEREADINGStatement.MODELELEMENT(liInd - 1)
                Me.VAQL.MODELELEMENTClause.PREBOUNDREADINGTEXT = ""
                Me.VAQL.MODELELEMENTClause.POSTBOUNDREADINGTEXT = ""
                Me.VAQL.MODELELEMENTClause.MODELELEMENTNAME = ""
                Call Me.VAQL.GetParseTreeTokensReflection(Me.VAQL.MODELELEMENTClause, lrModelElementNode)
                ...
                If Me.VAQL.FACTTYPEREADINGStatement.UNARYPREDICATEPART = "" Then
                    '----------------------------------------
                    'FactType is binary or greater in arity
                    '----------------------------------------
                ...
                Else
                    ...
                    'FactType is a unary FactType
                    '------------------------------
                    lrPredicatePart.PredicatePartText = Trim(Me.VAQL.FACTTYPEREADINGStatement.UNARYPREDICATEPART)
                End If
            ...
            lrQuestion = New tQuestion("Would you like me to create a Fact Type for '" & Trim(asOriginalSentence) & "'?", _
                                            pcenumQuestionType.CreateFactType, _
                                            True, _
                                            Nothing, _
                                            lrSentence, _
                                            Nothing, _
            ...
            'Get the FollowingReadingText if there is one.
            '-----------------------------------------------
            lrFactTypeReading.FollowingText = Trim(Me.VAQL.FACTTYPEREADINGStatement.FOLLOWINGREADINGTEXT)
...
#Region "HOUSEKEEPING"
    ...
    Private Function CheckIsSubtypeStatement(ByVal arSentence As Language.Sentence) As Boolean
        ...
        If arSentence.WordListResolved.Count = 4 Then
            If arSentence.WordListResolved.Count = 4 And _
               arSentence.WordListResolved(0).Sense = pcenumWordSense.Noun And _
               arSentence.WordListResolved(1).Word = "is" And _
               (arSentence.WordListResolved(2).Word = "a" Or arSentence.WordListResolved(2).Word = "an") And _
    ...
    Private Function CheckIsConceptStatement(ByVal arSentence As Language.Sentence) As Boolean
        ...
        If arSentence.WordListResolved.Count = 4 Then
            If arSentence.WordListResolved(0).Sense = pcenumWordSense.Noun And _
               arSentence.WordListResolved(1).Word = "is" And _
               arSentence.WordListResolved(2).Word = "a" And _
    ...
    Private Sub AskQuestionCreateConcept()
        ...
        If Me.ConfirmActions Then
            ...
            lrQuestion = New tQuestion("Would you like me to create a Concept for '" & Me.CurrentSentence.WordListResolved(3).Word & "'?", _
                                       pcenumQuestionType.CreateConcept, _
                                       True, _
                                       lasSymbol, _
                                       Me.CurrentSentence)
            If Me.QuestionHasBeenRaised(lrQuestion) Then
    ...
    Private Sub AskQuestionForgetAskedToAbortStatement(ByRef arSentence As Language.Sentence)
        ...
        lrQuestion = New tQuestion(lsQuestion, _
                                    pcenumQuestionType.ForgetAskedToAbortPlan, _
                                    True, _
                                    Nothing, _
                                    arSentence, _
                                    Nothing, _
    ...
    Private Sub AskQuestionCreateValueType(ByVal arValueType As FBM.ValueType, ByRef arSentence As Language.Sentence, Optional ByVal abInsertAtHeadOfQueue As Boolean = False, Optional ByRef arOriginalPlan As Brain.Plan = Nothing)
        ...
        lrQuestion = New tQuestion("Would you like me to create an Value Type for '" & arValueType.Name & "'?", _
                                         pcenumQuestionType.CreateValueType, _
                                         True, _
                                         lasSymbol, _
                                         arSentence, _
