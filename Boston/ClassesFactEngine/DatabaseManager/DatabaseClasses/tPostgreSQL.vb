...
Namespace FactEngine
    ...
    Public Class PostgreSQLConnection
        ...
        Public Overrides Function getForeignKeyRelationshipsByTable(ByRef arTable As RDS.Table) As List(Of RDS.Relation)
            ...
            Try
                ...
                While Not lrRecordset.EOF
                    '20210807-VM-Below is for SQLite
                    'Columns
                    '====================
                    ''id
        ...
        Public Overrides Function getColumnsByTable(ByRef arTable As RDS.Table) As List(Of RDS.Column)
            ...
            Try
                Dim lsSQL As String = "SELECT column_name,
                                              ordinal_position,
                                              data_type,
                                              is_nullable,
                                              column_default,
                ...
                While Not lrRecordset.EOF
                    ...
                    lbIsMandatory = lrRecordset("is_nullable").Data = "NO"
                    lrColumn = New RDS.Column(arTable, lsColumnName, Nothing, Nothing, lbIsMandatory)
                    lrColumn.OrdinalPosition = lrRecordset("ordinal_position").Data
                    lrColumn.DataType = New RDS.DataType
                    lrColumn.DataType.DataType = lrRecordset("data_type").Data
        ...
        Public Overrides Function getIndexesByTable(ByRef arTable As RDS.Table) As List(Of RDS.Index)
            ...
            Try
                ...
                While Not lrRecordset.EOF
                    ...
                    'index_name
                    'column_name
                    'is_primary (1=Primary, else 0)
                    'is_unique (1=Unique,else 0)
                    larColumn.Clear()

                    While Not lrRecordset.EOF

                        lsIndexName = lrRecordset("index_name").Data
                        lbIsUnique = CInt(NullVal(lrRecordset("is_unique").Data, 0)) > 0
                        If lrRecordset("is_primary").Data = 1 Then
                            lsQualifier = "PK"
                            lbIsPrimaryKey = True
                        Else
                            lsQualifier = "UC"
                        ...
                        If Not lrRecordset.EOF Then
                            If lrRecordset("index_name").Data <> lsIndexName Then
